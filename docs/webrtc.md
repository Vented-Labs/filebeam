...
Direct native peer connections expose the receiver's network address to the sender. A receiver must confirm interactively or pass `--accept-peer-address-exposure`; this flag accepts that exposure, not the safety or identity of the peer. `--webrtc-relay-only` instead requires a relay and skips that direct-peer consent. The native `webrtc` implementation accepts `turn:` UDP ICE URLs; TURN over TCP or TLS is unsupported.

Use `beam note open` for notes and `beam inbox download` for account-addressed transfers. See [Beam CLI](beam-cli.md) for commands and key-custody options.
