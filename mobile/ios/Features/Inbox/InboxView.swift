...
struct InboxKeyUnlockSheet: View {
    ...
    private func unlockAndReceive() async { do { let key: Data; if bundle.custody == .password { guard let envelope = bundle.encryptedPrivateKey else { error = "This password-custody key is unavailable."; return }; key = try await model.service.unwrapPasswordCustodyKey(envelope: envelope, password: password, userID: bundle.userID, publicKey: bundle.publicKey) } else { key = try await model.service.importSelfCustodyKey(exportedKey) }; password = ""; exportedKey = ""; await model.receiveInbox(item: item, privateKey: key); dismiss() } catch { self.error = model.message(error) } }
}
