...
struct KeySetupView: View {
    ...
    private func makeExportFile() { guard !exported.isEmpty else { return }; let url = FileManager.default.temporaryDirectory.appendingPathComponent("filebeam-recovery-key.txt"); do { try Data(exported.utf8).write(to: url, options: .atomic); exportURL = url } catch { self.error = model.message(error) } }
